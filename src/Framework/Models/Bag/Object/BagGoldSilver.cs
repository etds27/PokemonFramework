...
{
    public class BagGoldSilver : IBagObject
    {
        internal override MemoryQuery CurrentPocketQuery => new(0xCE65, 1, MemoryDomain.WRAM);
        internal override MemoryQuery SelectedItemQuery => new(0xD681, 1, MemoryDomain.WRAM);

        internal override IReadOnlyList<IPocketObject> Pockets
        {
            get => [
                pocketFactory.CreateObject(PocketType.Item, 0xD5B7, 0),
                pocketFactory.CreateObject(PocketType.Ball, 0xD5FC, 1),
                pocketFactory.CreateObject(PocketType.KeyItem, 0xD5E1, 2),
                pocketFactory.CreateObject(PocketType.TMHM, 0xD57E, 3),
                ];
        }
